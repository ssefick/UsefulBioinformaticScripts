...
#RGQ
RGQ_filter <- function(x, RGQ_value=RGQ_value, min_DP=min_DP, max_DP=max_DP) {
    ## Filter on RGQ > value; default is 30

    #out <- apply(geno(x)$RGQ>=value, 1, function(x)sum(x, na.rm=TRUE))==4

    out <- apply(geno(x)$RGQ >= RGQ_value & geno(x)$DP > min_DP & geno(x)$DP < max_DP, 1, function(x)sum(x, na.rm=TRUE))==4
...
#snps
#"QD < 2.0"; "MQ < 40.0"; "FS > 60.0"; "SOR > 3.0"; "MQRankSum < -12.5"; "ReadPosRankSum < -8.0"; "DP < 5"; "DP > 120"
snps <- function(x, min_DP=min_DP, max_DP=max_DP, GQ=GQ){

    out <- apply(info(x)$QD > 2 & info(x)$MQ > 40 & info(x)$FS < 60 & info(x)$SOR < 3 & info(x)$MQRankSum > -12.5 & info(x)$ReadPosRankSum > -8 & geno(x)$DP >= min_DP & geno(x)$DP <= max_DP & geno(x)$GQ >= GQ, 1, function(x)sum(x, na.rm=TRUE))==4
...
#"FS > 200.0"; "ReadPosRankSum < -20.0"; "SOR > 10.0"; "DP < 5"; "DP > 120"  
indels <- function(x, min_DP=min_DP, max_DP=max_DP, GQ=GQ){

    out <- apply(info(x)$FS < 200 & info(x)$SOR < 3 & info(x)$ReadPosRankSum > -8 & geno(x)$DP>=min_DP & geno(x)$DP<=max_DP & geno(x)$GQ>=GQ, 1, function(x)sum(x, na.rm=TRUE))==4
...
filters <- FilterRules(list(RGQ_filter=RGQ_filter, snps=snps, indels=indels))
